...
while(true){	 // 전체 주문받고 영수증 출력까지
	while(true)  // 주문 받는거 반복  
	{
			// 권종 선택 
		printf("권종을 선택하세요.\n");  
		printf("1. 주간권\n");
		...
		ticketList[totalOrderCount] = ticket; //1 또는 2  
		...
		ageList[totalOrderCount] = tickettype; // 티켓 타입 저장  
		...
		if(addOrder == 2) { //추가 주문 없을때 빠져나옴  
			break; // 전체while에 대한 break임. 2일때 반복문 종료 > 영수증 출력  
		} 
		totalOrderCount++;
	} 
	...
	for(int index = 0; index <= totalOrderCount; index++) {
		...
		if(numList[index] != 0){ //갯수  
			printf("%d", numList[index]);
		}
			
		if(totalList[index] >= 0){ //가격  
